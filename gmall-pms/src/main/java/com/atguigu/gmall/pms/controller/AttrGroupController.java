...
public class AttrGroupController {
    ...
    public Resp<List<AttrGroupVo>> queryGroup(@PathVariable("catId") Long catId) {
        List<AttrGroupVo> ls = attrGroupService.queryGroup(catId);
        return Resp.ok(ls);
    }

    @GetMapping("/withattr/{gid}")
    public Resp<AttrGroupVo> queryAttr(@PathVariable("gid") Long gid) {
        AttrGroupVo ls = attrGroupService.queryAttr(gid);
        return Resp.ok(ls);
    }

    @GetMapping("{catId}")
    public Resp<PageVo> queryGroup(QueryCondition queryCondition, @PathVariable("catId") Long catid) {
        PageVo page = attrGroupService.queryPage(queryCondition, catid);
        return Resp.ok(page);
    }
